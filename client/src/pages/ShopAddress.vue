...
<template lang="html">
  <div class="">
        ...
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
              ...
              <ul>
                <li v-for="(item,index) in addressListFilter" v-bind:class="{'check':checkIndex==index}" @click="checkIndex=index;selectedAddrId=item.addressId">
                  ...
                  <div class="addr-opration addr-del">
                    <a href="javascipt:;" class="addr-del-btn" @click="delAddressConfirm(item.addressId)">
                      <svg class="icon icon-del"><use xlink:href="#icon-del"></use></svg>
                    </a>
                  </div>
                  <div class="addr-opration addr-set-default">
                    <a href="javascipt:;" class="addr-set-default-btn" v-if="!item.isDefault" @click="setDefault(item.addressId)"><i>设为默认</i></a>
                  </div>
            ...
            <div class="shipping-addr-more">
              <a class="addr-more-btn up-down-btn" href="javascipt:;" @click="expand" :class="{'open':limit>3}" v-if="addressList.length>3">
                更多
    ...
    <modal :mdShow="isMdShow" @close="closeModal">
      ...
      <div slot="btnGroup">
        <a class="btn btn--m" href="javascipt:;" @click="delAddress">确认</a>
        <a class="btn btn--m btn--red" href="javascipt:;" @click="isMdShow=false">取消</a>
      </div>
    </modal>
    <modal :mdShow="isAddAddrShow" @close="closeModal">
      ...
      <div slot="btnGroup">
        <a class="btn btn--m" href="javascipt:;" @click="addAddress">确认</a>
        <a class="btn btn--m btn--red" href="javascipt:;" @click="isAddAddrShow=false">取消</a>
      </div>
    </modal>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    setDefault (addressId) {
      this.$http.post('/users/setDfault', { addressId })
      .then(res => {
        res = res.data
        if (res.status === '0') {
          this.init()
